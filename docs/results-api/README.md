...
> [!IMPORTANT] This schema defines how benchmarks are structured for the Prism
> backend to store, index, filter, and display. It does **not** directly mirror
> the raw output format of the upstream `llm-d-benchmark` Benchmark Report v0.2
> (`brv02`) files. For example, while raw reports from harnesses locate hardware
> accelerator definitions under `scenario.stack[].standardized.accelerator`, the
> Prism internal schema may represent or map these fields differently (e.g.,
> under `config` or as root-level descriptors) for query optimization.
...
## 2. Combined API Payload Schema (TypeScript Definitions)

The definitions for the API payload structure are defined in the codebase:

- **TypeScript Definitions:**
  [server/results/api.ts](../../server/results/api.ts)
    - `PrismResultPayload`: Root container representing the JSON payload content
      stored inside a result file inside the bucket.
    - `PrismResultContext`: Describes the GCS custom metadata context of a
      benchmark result.
...
- **`submitted_pending_review`**:
    - **Purpose:** Awaiting admin approval. Can be bypassed/auto-approved based
      on policy.
- **`public`**:
...
1. **Synchronous Execution**: The backend web server runs the validation logic
   (via
   [processSubmission](file:///usr/local/google/home/diamondburned/Projects/llm-d/llm-d-prism/server/results/processing.ts))
...
3. **Future Decoupling**: The processing wrapper is fully decoupled. In the
   future, the synchronous invocation inside
   [submitResultsHandler](file:///usr/local/google/home/diamondburned/Projects/llm-d/llm-d-prism/server/results/routes/submit.ts)
   can be removed in favor of an asynchronous background worker (e.g., GCS
   Object Eventarc trigger or Pub/Sub pull worker calling the same processing
   library).
...
> [!IMPORTANT] The submission status is solely determined from this object
> context (specifically the `submission_state` key), not which bucket it is in.
