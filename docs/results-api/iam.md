...
The server-side implementation is located in
[server/iam.ts](../../server/iam.ts) under
[validateGitHubUser](../../server/iam.ts#L105).
...
[tools/prism-iam](../../tools/prism-iam).
...
- **`edit`**: Fetches the GCS allowlist, opens it in the local `$EDITOR`
  ...
  changes were detected.
    > [!IMPORTANT] To prevent accidental lockouts, the command will abort and
    > fail fatally if the file is left empty after editing.
