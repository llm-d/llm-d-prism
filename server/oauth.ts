 ...
 */
export async function validateGitHubToken(token: string): Promise<{ username: string, permission: PermissionLevel }> {
...
oauthRouter.get('/api/auth/github/callback', async (req: Request, res: Response) => {
    ...
    try {
        ...
        // Redirect back to the frontend homepage with the token and state in the hash fragment
        res.redirect(`${getBaseUrl(req)}/#access_token=${encodeURIComponent(accessToken)}&state=${encodeURIComponent(state as string || '')}`);
    } catch (error) {
...
oauthRouter.get('/api/auth/github/me', async (req: Request, res: Response) => {
    ...
    try {
        const { username, permission } = await validateGitHubToken(token);
